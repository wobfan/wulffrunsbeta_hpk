 ...
 * @author nwulff
 * @since  08.12.2016
 * @version $Id: AbstractWRBServiceTester.java,v 1.2 2017/01/06 17:18:01 nwulff Exp $
 */
public abstract class AbstractWRBServiceTester {
    ...
    public static void setUpServer() throws Exception {
        // start the server
        server = WRBServer.startServer(new String[0]);
    }
     ...
     */
    protected <T extends Number> void assertEqualsPairs(List<Pair<T>> expected, List<Pair<T>> result) {
        int k = 0;
        assertTrue("size missmatch", expected.size() == result.size());
        for (Pair<T> p : result) {
            Pair<T> q = expected.get(k++);
            String debugmsg = format("%s != %s", q, p);
            assertEquals(debugmsg, q.x.doubleValue(), p.x.doubleValue(), eps);
            assertEquals(debugmsg, q.y.doubleValue(), p.y.doubleValue(), eps);
        }
    }
     ...
     * @param result   second pair list
     */
    protected <T extends Number> void assertEqualsPairs(String msg, List<Pair<T>> expected, List<Pair<T>> result) {
        int k = 0;
        assertTrue("size missmatch", expected.size() == result.size());
        for (Pair<T> p : result) {
            Pair<T> q = expected.get(k++);
    ...
    protected List<Pair<Double>> expectedValues(String fctName, Function fct, String fmt, double xmin, double xmax,
            double dx) {
        double x, y;
    ...
    public String expectedValue(String fctName, Function fct, String fmt, double xmin, double xmax, double dx) {
        List<Pair<Double>> values = expectedValues(fctName, fct, fmt, xmin, xmax, dx);
        StringBuilder sb = new StringBuilder();
        sb.append(fctName);
        sb.append(": [");
        for (Pair<Double> p : values) {
            sb.append(format(Locale.US, fmt, roundFmt(p.x, fmt)));
            sb.append(',');
            sb.append(format(Locale.US, fmt, roundFmt(p.y, fmt)));
            sb.append(',');
        }
    ...
    protected String submitHelloRequest(String message, String path) {
        target = target.queryParam("msg", message);
        target = target.path(format("%s/%s", AbstractWRBService.SERVICE, path));
        String responseMsg = target.request().accept(AbstractWRBService.TEXT_PLAIN).get(String.class);
        return responseMsg;
    }

    protected String submitHelloRequest(String message) {
        return submitHelloRequest(message, AbstractWRBService.HELLO_PATH);
    }
